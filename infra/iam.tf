resource "aws_iam_user" "druid_iam_user" {
  name = "${var.druid_config.cluster_name}-druid-usr"
}
...
resource "aws_iam_access_key" "druid_access_key" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
        echo access_key: ${aws_iam_access_key.druid_access_key.id} > ./storage/${var.druid_config.cluster_name}-config.txt
        echo secret_key: ${aws_iam_access_key.druid_access_key.secret} >> ./storage/${var.druid_config.cluster_name}-config.txt
    EOT
  }
}

resource "aws_iam_group" "druid_group" {
  name = "${var.druid_config.cluster_name}-druid-usr-gr"
}
